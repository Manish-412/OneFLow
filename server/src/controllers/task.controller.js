...
// Add members to task
exports.addTaskMembers = async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    // Create task_members table if it doesn't exist
    await pool.query(`
    ...
    const insertPromises = members.map(userId => 
      pool.query(`
    ...
    const addedCount = results.filter(r => r.rows.length > 0).length;

    res.json({
      success: true,
      message: `Successfully added ${addedCount} member(s) to the task`,
      addedCount
    });
  } catch (error) {
    console.error('Error adding task members:', error);
    res.status(500).json({ success: false, message: 'Failed to add members to task' });
  }
};
...
exports.getTaskMembers = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch task members' });
  }
};
